...
async function captureEachCamera(page) {
...
async function scrapePage() {
    ...
    let paths = await captureEachCamera(page);
    await browser.close();
...
const tweet = async(paths, title) => {
  ...
  const mediaIds = await Promise.all(selectedPaths.map(path => rwClient.v1.uploadMedia(`./${path}`))); // upload each image
...
async function main() {
  ...
  const {paths, title} = await scrapePage();
